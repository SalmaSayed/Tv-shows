		...
		_proto.init = function init() {
			...
			this.header.items.forEach(function(line) {
				line.items.forEach(function(c) {
					...
					if (c.resizable || (c.header && c.header.resizable)) _this.hasResizableColumns = true;
				...
				line.children.forEach(function(columnInstance, i) {
					...
					var _loop = function _loop(l) {
						...
						if (header) {
							...
							var customWidth =
								header.data.width || instance.state.colWidth[c.uniqueColumnId] || header.data.defaultWidth;
								...
								resizer = VDOM.createElement("div", {
									...
									onMouseMove: function onMouseMove(e) {
										...
										resizeOverlayEl.style.left =
											headerCell.getBoundingClientRect().left - scrollAreaEl.getBoundingClientRect().left + "px";
										...
										captureMouse(
											...
											function(e) {
												...
												if (!header.set("width", width))
													instance.setState({
														colWidth: _objectSpread(
															{},
															instance.state.colWidth,
															((_objectSpread2 = {}), (_objectSpread2[c.uniqueColumnId] = width), _objectSpread2)
														)
													});
											}
										);
									...
									onDoubleClick: function onDoubleClick(e) {
										...
										instance.setState({
											colWidth: _objectSpread(
												{},
												instance.state.colWidth,
												((_objectSpread3 = {}), (_objectSpread3[c.uniqueColumnId] = null), _objectSpread3)
											)
										});
									}
								});
...
var GridComponent =
	...
	(function(_VDOM$Component) {
		...
		_proto2.render = function render() {
			...
			var colWidth = instance.state.colWidth;
						...
						children.content.map(function(_ref2) {
							...
							return VDOM.createElement(
								...
								content.map(function(_ref3, cellIndex) {
									...
									var cellected = index == cursor && cellIndex == cursorCellIndex;
			...
			var content = []; //instance.records holds the record count after filtering
		...
		_proto2.handleKeyDown = function handleKeyDown(e) {
			...
			switch (e.keyCode) {
				...
				case KeyCode.down:
					for (var cursor = this.state.cursor + 1; ; cursor++) {
						var record = this.getRecordAt(cursor);
						if (!record) break;
				...
				case KeyCode.up:
					for (var _cursor = this.state.cursor - 1; _cursor >= 0; _cursor--) {
						var _record = this.getRecordAt(_cursor);

						if (!_record) break;
						if (_record.type != "data") continue;
